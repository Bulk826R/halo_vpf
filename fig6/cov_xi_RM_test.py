...
t = 00 # number density ratio of R to D
...
for i in range(len(cf_jack[0])):
    ...
    low = np.percentile(cf_jack[:, i], 15.85)
    high = np.percentile(cf_jack[:, i], 84.15)
...
for i in range(len(cf_poi[0])):
    ...
    low = np.percentile(cf_poi[:, i], 2.3)
    high = np.percentile(cf_poi[:, i], 97.7)
...
np.savetxt('/home/bulk826/Desktop/Stanford/Research3/data/redMapper/xi_cov_{}.txt'.format(len_poi), cov_mat)
...
for i in range(len(cf_jack)):
    t1 = np.dot(inv_covmat, (cf_jack[i, id_cf] - CF_mean))
    t2 = np.dot(cf_jack[i, id_cf] - CF_mean, t1)
    jack_chi2.append(t2)
    
jack_chi2 = np.asarray(jack_chi2)

np.savetxt(baseDir+'/redMapper/data_chi_2_xi.txt', [data_chi2])
np.savetxt(baseDir+'/redMapper/rand_chi_2_xi.txt', rand_chi2)
np.savetxt(baseDir+'/redMapper/jack_chi_2_xi.txt', jack_chi2)
...
ncen = 200
len_bins = 51
data_chi2 = np.loadtxt(baseDir+'/redMapper/data_chi_2_xi.txt')
rand_chi2 = np.loadtxt(baseDir+'/redMapper/rand_chi_2_xi.txt')
jack_chi2 = np.loadtxt(baseDir+'/redMapper/jack_chi_2_xi.txt')
'''
print('data chi2 = ', data_chi2)
...
fig.savefig('/home/bulk826/Desktop/Stanford/Research3/figures/paper/draft/cov_xi_RM_test_{0}_{1}bins.png'.format(len_poi, len_bins), dpi = 400, bbox_inches = 'tight')
plt.show() 