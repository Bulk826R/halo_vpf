...
bine = np.logspace(-2.7, -0.43, 51)
binw = bine[1:] - bine[:-1]
binc1 = (bine[1:] + bine[:-1]) / 2

bine = np.logspace(-1.5, -0.385, 51)
binw = bine[1:] - bine[:-1]
binc2 = (bine[1:] + bine[:-1]) / 2

bine = np.logspace(-1.35, -0.29, 51)
binw = bine[1:] - bine[:-1]
binc4 = (bine[1:] + bine[:-1]) / 2

bine = np.logspace(-1.3, -0.25, 51)
binw = bine[1:] - bine[:-1]
...
np.savetxt('/oak/stanford/orgs/kipac/users/ycwang19/VPF/SZ/chi_haver_{}.txt'.format(len_poi), cov_mat)
...
fig.savefig('/oak/stanford/orgs/kipac/users/ycwang19/VPF/figures/chi_haver_{}.png'.format(len_poi), dpi = 400, bbox_inches = 'tight')
plt.show()
