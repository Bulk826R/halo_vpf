...
CDF1_std = np.std(CDF1, axis = 0) * np.sqrt(Nhalos - 1)
CDF2_std = np.std(CDF2, axis = 0) * np.sqrt(Nhalos - 1)       

res = np.vstack((CDF1_mean, CDF2_mean, CDF1_std, CDF2_std, cdf01, cdf02)).T
np.savetxt('/oak/stanford/orgs/kipac/users/ycwang19/VPF/SZ/knn_loo/knn_loo.txt', res)