...
def create_bins(kNN):
    ...
    bine = np.logspace(np.log10(30), np.log10(220), 51)
    binw = bine[1:] - bine[:-1]
    bins[:, 2] = (bine[1:] + bine[:-1]) / 2
    #4NN
    bine = np.logspace(np.log10(30), np.log10(220), 51)
    binw = bine[1:] - bine[:-1]
    bins[:, 3] = (bine[1:] + bine[:-1]) / 2
...
for sim_number in range (1000): # We only need one sim for the jackknife calculation
    ...
    fname = '/oak/stanford/orgs/kipac/users/ycwang19/VPF/SZ/knn_data/data_'+str(sim_number)+'.txt'
