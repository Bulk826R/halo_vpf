...
ax2.scatter(binc[id_poi], dCF, lw = 0.8, color = 'crimson', alpha = 0.5, facecolors = 'None',
            marker = 'D', s = 82, rasterized = True)
ax2.plot(binc[id_poi], dCF, lw = 1.6, alpha = 0.8, color = 'crimson', 
         linestyle = '-.')

ax2.fill_between(binc[id_jack], dCF_low, dCF_high, alpha = 0.3, 
                 color = 'crimson')

V1 = np.min(binc[id_poi])
V2 = np.max(binc[id_poi])
x = np.linspace(V1, V2, 10)
y = np.ones_like(x)
ax2.axhline(y=0, lw = 1.2, dashes = (5, 2.4), color = 'royalblue', alpha = 0.8)
...
fig.savefig('/home/bulk826/Desktop/Stanford/Research3/figures/paper/draft/kmean_xi_jack_{0}_{1}bins.pdf'.format(len_poi, len_bins), dpi = 400, bbox_inches = 'tight')
plt.show()