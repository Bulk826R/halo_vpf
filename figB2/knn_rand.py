...
for sim_number in range (1000): # We only need one sim for the jackknife calculation
    ...
    '''
    X = np.random.rand(Nhalos) * boxsize
    Y = np.random.rand(Nhalos) * boxsize
    Z = np.random.rand(Nhalos) * boxsize
    ...
    fname = '/oak/stanford/orgs/kipac/users/ycwang19/VPF/SZ/knn_qui/rand_'+str(sim_number)+'.txt'
